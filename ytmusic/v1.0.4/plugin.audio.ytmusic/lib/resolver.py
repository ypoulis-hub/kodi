...
def _init_resolver():
    ...
    # On Linux, offer to download yt-dlp automatically
    if os.name != 'nt':
...
def _auto_install_ytdlp():
    """Offer to download yt-dlp binary on Linux systems."""
    ...
    install_dir = '/storage/bin'  # LibreELEC
    ...
    try:
        ...
        if result.returncode != 0:
            pbar.close()
            log('yt-dlp download failed: {}'.format(result.stderr[:200]))
            dialog.ok('YTMusic', 'Download failed: {}'.format(result.stderr[:100]))
            return False
